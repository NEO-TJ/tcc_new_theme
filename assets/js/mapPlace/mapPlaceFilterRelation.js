...
function ChangeDaterange(picker) {
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            setSelectElementOfOrg(result.dsOrg, $('select#orgId'));
        }
    });
}
...
function setSelectElementOfOrg(dataSet, $selector) {
    $selector.empty();
    $selector.append('<option value="0">เลือกทั้งหมด</option>');
    for (let i = 0; i < dataSet.length; i++) {
        $selector.append('<option value="' + dataSet[i].FK_Org + '">' + dataSet[i].department + '</option>');
    }
}